const BorderMagic = ({
  ...
  return (
    <button
      ...
      {/* bg and text color */}
      <span
        className={`inline-flex gap-2 h-full w-full cursor-pointer items-center justify-center rounded-lg bg-white  border-none  text-black dark:bg-slate-950 px-4 lg:px-7 text-sm md:text-base lg:text-lg font-medium dark:text-white backdrop-blur-3xl ${otherClasses}`}>
        {position === "left" && icon}
        {title}
        {position === "right" && icon}
      </span>
